...
// === Конфигурация ===
const GLOBAL_OBJECTS_DIR = ".";
const OUTPUT_FILE = "./globalObjects.d.ts";
...
function generateDtsFile(typesData) {
  ...
  for (let objName in typesData) {
    ...
    dtsContent += `/**\n`;
    dtsContent += ` * Глобальный объект ${objName}\n`;

    if (objName != "Math") dtsContent += ` */\ndeclare const ${objName}: {\n`;
    else dtsContent += ` */\ninterface ${objName} {\n`;
 ...
 * Основная логика
 */
function globalObjectsMdToJSDoc(inputDir) {
  try {
    const result = [];
...
module.exports = globalObjectsMdToJSDoc
